...
def evaluate():
    ...
    for entry in DATASET:
        ...
        # --- DEBUG: show what was extracted and found ---
        if entry["name"] == "lighthouse":
            print("\n  --- DEBUG: extracted facts ---")
            for f in facts:
                print(f"    {f['entity']} | {f['attribute']} | {f['value']}")
            print("\n  --- DEBUG: detected contradictions ---")
            for c in found:
                print(f"    {c['fact_a']['entity']}={c['fact_a']['value']} "
                      f"VS {c['fact_b']['entity']}={c['fact_b']['value']}")
            print()
